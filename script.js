...
const App = {
    ...
    bindEvents() {
        ...
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            UIModule.elements.voiceBtn.addEventListener('click', () => {
                try {
                    recognition.start();
                    // Visual feedback
                    UIModule.elements.voiceBtn.classList.add('text-red-500', 'animate-pulse');
                    UIModule.elements.voiceBtn.innerHTML = "<i class='bx bx-stop-circle'></i>";
                } catch (e) {
            ...
            recognition.onresult = (event) => {
                ...
                // Intelligent Parsing (Simple)
                // If user says "Rapat jam 3 sore", we could parse time. For now, just title.
                document.getElementById('task-title').value = transcript;
                
                // Stop animation
                UIModule.elements.voiceBtn.classList.remove('text-red-500', 'animate-pulse');
                UIModule.elements.voiceBtn.innerHTML = "<i class='bx bx-microphone'></i>";
            ...
            recognition.onerror = () => {
                UIModule.elements.voiceBtn.classList.remove('text-red-500', 'animate-pulse');
                UIModule.elements.voiceBtn.innerHTML = "<i class='bx bx-microphone'></i>";
                UIModule.showToast("Gagal mendengar suara.", "error");
            };
            ...
            recognition.onend = () => {
             ...
             UIModule.elements.voiceBtn.style.display = 'none'; // Hide if not supported
        }
